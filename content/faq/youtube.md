...
To delete the original channel (from the web browser):
Navigate to the **Wallet** page (click on your balance in the top right). Under **"Transactions"** there is a dropdown menu labelled as **"Type"**. Click on it and then select **"Channel"**. You may need to navigate the pages below in order to find the rows to delete. Some results may be filtered with the **Status** (Active, Historical, All). You will find some rows with a trash can icon near the name of the channel. **Make sure it is the right channel to delete by clicking on the name first, or you might end up deleting the wrong channel**. If you are sure, use the trash can icon to remove the row with the transaction. You might be asked to manually confirm the operation. This process is permanent, so be careful not to delete the synced channel you want to keep.
